...
# ============================
# OpenAI-only edition (no OpenRouter required)
import logging
...
def install_launcher_deps() -> None:
    subprocess.run(
        [sys.executable, "-m", "pip", "install", "-q", "openai>=1.0.0", "requests", "httpx"],
        check=True,
    )
...
def ensure_claude_code_cli() -> bool:
    """Best-effort install of Claude Code CLI for Anthropic-powered code edits (optional)."""
...
# OPTIONAL (kept for compatibility with code that checks it, but unused in OpenAI-only mode)
...
# Model defaults now assume OpenAI model names
MODEL_MAIN = get_cfg("OUROBOROS_MODEL", default="gpt-5.2", allow_legacy_secret=True)
MODEL_CODE = get_cfg("OUROBOROS_MODEL_CODE", default=MODEL_MAIN, allow_legacy_secret=True)
...
DIAG_HEARTBEAT_SEC = _parse_int_cfg(get_cfg("OUROBOROS_DIAG_HEARTBEAT_SEC", default="30", allow_legacy_secret=True), default=30, minimum=0)
DIAG_SLOW_CYCLE_SEC = _parse_int_cfg(get_cfg("OUROBOROS_DIAG_SLOW_CYCLE_SEC", default="20", allow_legacy_secret=True), default=20, minimum=0)

# Export env for subprocesses/workers
...
os.environ["OUROBOROS_MODEL_CODE"] = str(MODEL_CODE or MODEL_MAIN or "gpt-5.2")
if MODEL_LIGHT:
        ...
        try:
            ...
            if idle_sec >= HARD_TIMEOUT_SEC:
                ...
                if st.get("owner_chat_id"):
                    send_with_budget(int(st["owner_chat_id"]), f"⚠️ Task stuck ({int(total_sec)}s). Restarting agent.")
                reset_chat_agent()
                soft_warned = False
                continue
            if idle_sec >= SOFT_TIMEOUT_SEC and not soft_warned:
                ...
                st = load_state()
                if st.get("owner_chat_id"):
                    send_with_budget(int(st["owner_chat_id"]), f"⏱️ Task running {int(total_sec)}s, last progress {int(idle_sec)}s ago.")
        except Exception:
...
def _handle_supervisor_command(text: str, chat_id: int, tg_offset: int = 0):
    ...
    if t == "/panic":
        kill_workers()
        send_with_budget(chat_id, "🛑 Panic: workers killed.")
        return True
    if t == "/status":
        st = load_state()
        send_with_budget(chat_id, f"Workers={len(WORKERS)} pending={len(PENDING)} running={len(RUNNING)} spent_usd={st.get('spent_usd',0)} calls={st.get('spent_calls',0)}")
        return True
    if t == "/restart":
        ok, msg = safe_restart(reason="manual_restart", unsynced_policy="rescue_and_reset")
        send_with_budget(chat_id, f"♻️ Restart: {ok} {msg}")
        return True
    if t.startswith("/review"):
        queue_review_task(reason="manual_review")
        send_with_budget(chat_id, "🔎 Review queued.")
        return True
    if t.startswith("/evolve"):
        # delegate to queue logic in supervisor.queue via state flags; keep as in upstream behavior
        send_with_budget(chat_id, "🧬 Evolution command received. (Use upstream evolve controls.)")
        return True
    return False

while True:
    try:
        # Main Telegram polling loop lives in supervisor.telegram; we just dispatch events.
        # Use dispatch_event from supervisor.events (upstream).
        ...
        log.exception("Launcher main loop error")
        time.sleep(1.0)
